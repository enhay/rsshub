...
const news = async () => {
  ...
  try {
    ...
    logger.error(`wanplus chrome error: ${JSON.stringify(error)} ${error.stack}`);
    return [];
  } finally {
    browser.close();
  }
...
async function runWanplusCache() {
  const result = await news();
  cache = result;
}

runTimer.push(runWanplusCache);
runWanplusCache();
module.exports = async () => {
  try {
    return cache;
  } catch (error) {
    logger.error(`wanplus router error: ${JSON.stringify(error)} ${error.stack}`);
    return [];
  }
};
